...
export class OrderRepositoryDatabase implements OrderRepository {
    ...
    async getALLOrders(): Promise<Order[] | undefined> {
        ...
        return orders?.map((order) => Order.restore(order.order_id, order.products, order.status));
    }
    ...
    async updateOrder(order: Order): Promise<Order> {
        ...
        try {
            ...
            await trx.commit();
            return Order.restore(updatedOrder.order_id, order.products, updatedOrder.status);
        } catch (error) {
            await trx.rollback();
            throw error;
        }
    ...
    async getOrderById(order_id: string): Promise<Order | undefined> {
        ...
        return Order.restore(order.order_id, orderProducts, order.status);
    }
}
