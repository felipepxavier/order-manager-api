...
export class ProductRepositoryDatabase implements ProductRepository {
  ...
  constructor(readonly databaseConnection: DatabaseConnection<Knex>) {
    ...
    async getALLProductsByCategory(category: string): Promise<Product[] | undefined> {
        return await this.db<Product>("product").where({ category }).select("*");
    }
    async createProduct(product: Product): Promise<Product> {
        const [insertedProduct] = await this.db<Product>("product").insert(product).returning("*");
    ...
    async updateProduct(product: Product): Promise<Product> {
        const [updatedProduct] = await this.db<Product>("product").where({ product_id: product.product_id }).update(product).returning("*");
    ...
    async removeProduct(product_id: string): Promise<Product> {
        const [removedProduct] = await this.db<Product>("product").where({ product_id }).delete().returning("*");
    ...
    async getALLProducts(): Promise<Product[]> {
        const products = await this.db<Product>("product").select("*");
